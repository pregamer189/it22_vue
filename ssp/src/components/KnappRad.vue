<template>
    <div class="knapprad">
        <button v-for="knapp in knappar" :key="knapp" @click="spelarval" class="alternativ">{{ knapp }}</button>
    </div>
</template>
...
watch(() => props.reset, () => {
    if (props.reset) {
        ...
        for (let b of buttons) {
            b.classList.remove('spelarval', 'datorval');
            b.title = '';
        }
    }
});
...
function spelarval(e) {
    let buttons = document.getElementsByClassName('alternativ');
    for (let b of buttons) b.classList.remove('spelarval');
